...
interface ActivePipelinePageV3Props {
  onBack: () => void;
...
// Updated metrics: Accelerate (double width) and Stagnating
const healthMetrics = [
  { 
    label: "Accelerate", 
    value: "30", 
...
const ActivePipelinePageV3 = ({ onBack, onTierClick }: ActivePipelinePageV3Props) => {
  return (
    <div className="py-12 bg-page-background min-h-full">
      <div className="max-w-[1100px] mx-auto px-8">
        ...
        >
          {/* Top Row - Featured Accelerate and Ask AI */}
          <div className="grid grid-cols-2 gap-8 pb-8 border-b border-border">
            {/* Featured Accelerate Panel - Icon aligned to top */}
            <motion.div 
            ...
            >
              <div className="flex items-start gap-5">
                <div className="p-4 rounded-2xl bg-success/10 flex-shrink-0">
                  <Target className="h-10 w-10 text-success" />
                </div>
                <div>
                  <div className="flex items-baseline gap-3">
                    <span className="text-5xl font-bold text-foreground">30</span>
                    <span className="text-xl font-semibold text-foreground">Accelerate</span>
                  </div>
                  <p className="text-muted-foreground mt-1 mb-3">Leaders ready for executive roles</p>
                  <p className="text-sm text-muted-foreground mb-4">
                    Top performers identified as showing the strongest leadership signals, ready for executive sponsorship and stretch assignments.
                  </p>
                  <button 
                    onClick={() => onTierClick?.("accelerate")}
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-success bg-success/10 hover:bg-success/20 transition-colors border border-success/20"
                  >
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    Get personalized insights about your pipeline, stagnating leaders, and development recommendations.
                  </p>
                </div>
              </div>
              <div className="flex justify-end">
                <AIAssistantBadge 
                  context="Leadership Pipeline V3 - Signal Map view with Accelerate, Nurture, and Monitor tiers"
                  suggestedPrompts={[
                    "Which leaders are at risk of stagnating?",
                    "What development actions should we prioritize?",
                    "Show me the fastest-rising leaders",
                    "How can we improve pipeline conversion rates?"
                  ]}
                />
              </div>
            </motion.div>
          ...
          {/* Pipeline Talent Quadrant */}
          <div className="pt-8">
            
            {/* Quadrant Visualization with labels inside */}
            <div className="relative">
              ...
              <div className="relative h-[600px] rounded-xl border border-border">
                ...
                {[
                  ...
                  { name: "Sarah Mitchell", tier: "Projected", x: 35, y: 18, timeInRole: "1.5 years", image: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=100&h=100&fit=crop&crop=face", insight: "Exceptional trajectory—reached Projected tier in just 18 months." },
                  { name: "Sofia Rodriguez", tier: "Advancing", x: 25, y: 35, timeInRole: "2.5 years", image: "https://images.unsplash.com/photo-1580489944761-15a19d654956?w=100&h=100&fit=crop&crop=face", insight: "Rapid signal growth over 2.5 years." },
                  
                  // STAGNATING (top-right): High signal, long tenure (5+ years)
                  { name: "Daniela Chen", tier: "Projected", x: 62, y: 15, timeInRole: "6 years", image: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop&crop=face", insight: "⚠️ Strong signals but 6 years without promotion." },
                  { name: "Marcus Thompson", tier: "Advancing", x: 72, y: 40, timeInRole: "7 years", image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face", insight: "⚠️ Strong people leadership signals but plateauing." },
                  { name: "Tom Bradley", tier: "Projected", x: 82, y: 28, timeInRole: "8 years", image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop&crop=face", insight: "⚠️ Critical: 8 years at Projected tier." },
                  
                  // DEVELOPING (bottom-left): Lower signal, early tenure (0-5 years)
                  { name: "James Wilson", tier: "Detected", x: 28, y: 58, timeInRole: "2 years", image: "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=100&h=100&fit=crop&crop=face", insight: "Early signals emerging after 2 years." },
                  { name: "Liam O'Connor", tier: "Detected", x: 18, y: 72, timeInRole: "1 year", image: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=100&h=100&fit=crop&crop=face", insight: "Newest pipeline entrant at 1 year." },
                  
                  // STEADY LEADERS (bottom-right): Consistent signal, long tenure (5+ years)
                  { name: "Priya Sharma", tier: "Advancing", x: 68, y: 55, timeInRole: "6.5 years", image: "https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?w=100&h=100&fit=crop&crop=face", insight: "Reliable contributor with consistent execution." },
                  { name: "Elena Vasquez", tier: "Advancing", x: 58, y: 65, timeInRole: "7.5 years", image: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&h=100&fit=crop&crop=face", insight: "Steady progression with strong execution signals." },
                ].map((person, index) => {
                  const tierColors: Record<string, string> = {
                    Projected: "ring-3 ring-success/60",
                    Advancing: "ring-3 ring-warning/60",
                    Detected: "ring-3 ring-slate-400/60",
                  };
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    <span className="font-semibold text-destructive">3 leaders in the Stagnating quadrant</span> (Daniela, Marcus, Tom) have strong signals but long tenure without progression—consider executive sponsorship or stretch assignments to re-engage. 
                    <span className="font-semibold text-success ml-1">Rising Stars</span> Sarah and Sofia are prime candidates for accelerated development tracks.
                  </p>
                </div>
              </div>
            </motion.div>
        ...
        {/* Tier Cards Section - V7 Style Layout */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className="flex gap-2">
                      {stage.topPeople.slice(0, 3).map((person, idx) => (
                        <img 
                          key={person.name}
                          src={person.image}
                          alt={person.name}
                    ...
                    <div className={`ml-auto ${stage.countBg} ${stage.borderColor} border px-3 py-1.5 rounded-lg`}>
                  ...
                  <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium text-primary bg-primary/5 hover:bg-primary/10 transition-colors border border-primary/20">
                    View pipeline
...
export default ActivePipelinePageV3;